...
class DeepSeekNaturalLanguageProvider(DeepSeekReminderIntentProvider):
    ...
    def _router_prompt(now: datetime, timezone: str) -> str:
        ...
        return (
            ...
            "medication_cycle 允许槽位 medicine_name、dose_text、frequency、time_of_day、times；"
            "每天、每日、长期或连续服药都必须选择 medication_cycle，不得选择 reminder。"
            "同一天有多个服药时刻时，time_of_day 填第一个时刻，并用 times 数组保存全部时刻；"
