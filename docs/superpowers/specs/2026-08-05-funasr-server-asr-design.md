...
- App 优先录制单声道、16 kHz、16-bit PCM WAV；服务端兼容真机实际输出并统一归一化。
...
- 请求和检查麦克风权限。
- 请求录制 16 kHz、单声道、16-bit PCM WAV，并读取插件回报的实际参数。
...
- 接受 8 至 48 kHz、1 至 2 声道、16-bit PCM；FunASR 容器内的请求适配层在模型推理前显式转换为 16 kHz 单声道，Django 和模型都不隐式猜测音频格式。
- 音频时长为 0.3 至 20 秒，音频文件最大 4 MiB，Nginx/multipart 请求体最大 5 MiB；时长和 `audio_duration_ms` 由服务端根据 WAV data chunk 计算。
- V1 全局只允许一个识别请求，使用带唯一持有者 token 的 Redis 租约做非阻塞准入；租约已占用时不排队，返回 `429 asr_busy` 和 `Retry-After`。租约在 25 秒后自动过期，并且只能由持有者释放，避免进程崩溃永久占槽或旧请求释放新租约。
- 每个用户同一时间最多有一个识别请求；重复提交同样返回 `asr_busy`。
...
容器使用 `restart: unless-stopped`，启动时加载并预热模型，健康检查只有在模型可接受请求后才成功。连接拒绝或未就绪映射为 `asr_unavailable`，模型调用超过总超时映射为 `asr_timeout`。监控必须记录容器重启次数、启动耗时、CPU 和常驻内存。

模型、FunASR 和运行时版本必须在依赖与镜像中固定，不能在线上启动时自动升级。模型文件放在持久化只读缓存卷中，生产环境不得在每次容器重启时重新下载。FunASR 源码许可证不能代替模型权重许可证；固定模型 revision 时必须记录权重许可证和公开发布适用性。
...
- Nginx 在把请求交给 Django 前执行 5 MiB 请求体上限；DRF 使用 `MultiPartParser`，在访问 `request.data` 前执行认证和限流。
...
| FunASR 单并发已占用 | 显示可重试错误，保留文字输入 | 不排队，返回 `429 asr_busy` 和 `Retry-After` |
| FunASR 未启动或连接失败 | 显示服务暂不可用，保留文字输入 | 返回 `503 asr_unavailable` 并删除临时音频 |
...
- WAV 校验与归一化单元测试：合法 PCM、错误 RIFF 头、8/16/44.1/48 kHz、单/双声道、时长上下界和文件/请求体上限。
- Provider 契约测试：成功、空文本、超时、连接失败、非法响应和敏感日志检查。
- API 测试：未认证、限流、跨用户全局单并发、单用户重复提交、稳定错误码、重试重新推理和临时文件删除。
...
- 使用取得授权的 20 条普通话提醒录音，至少 18 条保留正确的提醒事项和时间关键词，并能生成预期草稿；同时记录 CER 作为后续模型对比基线。
- 验证 `ct-punc` 输出的逗号、句号和问号不会破坏本地提醒解析，保留带标点 transcript 的回归样本。
...
多副本、负载均衡、短队列、容量指标和自动扩展统一记录在 `2026-08-05-smart-reminder-v2-enhancements.md`，不纳入 V1 实施范围。
