...
- 现有 `quantity` 仅表示完整包装数（盒、瓶、袋、支等），不将“每盒几片”或“剩余几片”混入同一字段。
- V2 在库存批次上增加包装单位、完整包装数、每包装含量、最小计量单位和散装剩余量。
- 剩余总量由系统根据“完整包装数 × 每包装含量 + 散装剩余量”计算，不单独存储可以被推导的总量。
- 录入页默认只要求完整包装数；用户开启“记录精确剩余量”后，再展开每包装含量和散装剩余量，避免普通录入过于拥挤。
- 服药成功后优先扣减已开封包装的最小单位；手工修正、家庭并发更新和低库存提醒都必须保留审计记录。
- 数据迁移时将旧 `quantity` 映射为完整包装数，每包装含量保持未知，禁止根据规格文字自动猜测并回填历史数据。
