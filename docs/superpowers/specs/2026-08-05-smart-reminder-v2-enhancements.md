...
2026-08-13 已进入第一阶段实现：

- 个人药箱与家庭药箱明确分离，录入默认进入个人药箱。
- 一个账号只能加入一个家庭，每个家庭最多 10 人；采用管理员和普通成员两级权限。
- 管理员可生成 6 位、24 小时、单次使用的邀请码；支持转让管理员、移出成员、退出与解散家庭。
- 家庭库存保留批次模型，普通成员可查看、录入和修正，只有管理员可删除；共享修改带版本号并记录审计。
- 家庭药品可用于创建个人用药计划，但计划、剂量、服药状态和历史仍保持私有；离开家庭后相关计划自动暂停。
- 家庭临期事项在所有成员的行动中心可见，任一成员处理后全家同步消失。
- 本阶段仅实现 App 内同步，不包含 APNs 家庭远程推送、服药状态授权共享和跨成员升级通知。
...
2026-08-14 已提前完成第一阶段：

- `quantity` 继续只表示完整包装数（盒、瓶、袋、支等），不将“每盒几片”或“剩余几片”混入同一字段。
- 库存批次已经增加包装单位、每包装含量、最小计量单位和已开封剩余量；剩余总量按“完整包装数 × 每包装含量 + 已开封剩余量”实时推导，不重复存储。
- 录入页默认只要求完整包装数；开启“精确剩余量”后，再展开每包装含量和已开封剩余量，避免普通录入过于拥挤。
- 周期用药确认会创建结构化剂量和服药实例。确认已服后优先扣减已开封余量，再按有效期最早的批次扣减；重复动作不重复扣减。
- 同一周期用药计划已支持多个每日服药时刻；明确输入多个时刻时分别生成实例和 iOS 重复通知，只提供“每天几次”时必须追问具体时间，禁止默认推断早中晚。
- 库存不完整、单位不匹配、数量不足、药品未关联或家庭权限失效时，只记录服药，不猜测、不部分扣减，并返回明确状态。
- 旧 `quantity` 已保留为完整包装数，每包装含量保持未知，禁止根据规格文字自动猜测并回填历史数据。

仍保留在后续版本：

- 为历史批次提供手工精确数量校准和变更审计入口。
- 支持撤销“已服”并依据扣减账本恢复原批次库存。
- 支持按最小计量单位配置低库存阈值和补货提醒。
- 完成家庭多成员并发扣减压测、审计展示和冲突处理真机验收。
...
- 家庭邀请、关系解除、个人药箱和共享药箱边界已完成第一阶段实现。
- 支持用户明确授权家人查看服药完成状态，不默认共享疾病备注、用量文字或完整历史。
- 代录药品、共享批次和库存审计已完成；跨成员升级提醒仍待后续实现。
