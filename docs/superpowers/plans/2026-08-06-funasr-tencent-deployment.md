...
Use `smart-reminder-funasr:${APP_VERSION}` for both model initialization and inference. Keep the model volume persistent and private; do not expose port 8000. Configure the complete ASR environment, one global and one per-user concurrent request, and the fixed Nginx proxy IP. Accept only `8 <= ASR_TIMEOUT_SECONDS <= 20`, split the HTTPX phase budget into connect 2, pool 1, write 4, and read `total-7` seconds, check elapsed monotonic time after completion, and keep Gunicorn at `30` and Nginx at `35` seconds. Do not claim a mathematically strict total deadline for arbitrary streamed chunks. With production-default `OCR_ENABLED=false`, authenticated requests to all `/api/v1/ocr/*` endpoints must return `503 {"code":"ocr_disabled"}`, while unauthenticated requests must still return `401` before the feature-state check. The deploy sequence must validate configuration/revision/logging, capture the previous API and FunASR images, build candidates while the old API/Nginx stay live, start PostgreSQL/Redis, conditionally stop OCR services, check the pinned model marker, mark FunASR disrupted before stopping or replacing it, initialize a missing or stale cache with `--no-deps`, start FunASR, wait for `/health`, pass a real synthetic Mandarin WAV transcription, run candidate dependency preflight, migrate, mark API replaced, then replace API/worker. An ERR handler must preserve the original status and, after any affected step fails, disable its own trap, restore and health-check old FunASR first, then restore and health-check old API/worker. Validate production Nginx in a one-off container before force-recreating the live proxy and clear the trap only on complete success. Never roll back or delete database/model volumes. Only `OCR_ENABLED=true` starts the `ocr` profile and performs MinIO init plus OCR smoke before Nginx recreation.
