...
4. 录制短普通话提醒，确认转写可编辑且必须经过现有人工确认页。
5. 保持 `OCR_ENABLED=false` 时确认 API 和语音流程不依赖 MinIO 或 OCR Worker。
6. 显式恢复 OCR 后，拍摄药盒正面和有效期区域，确认上传地址 host 为 `files.aipupu.cloud`，等待 OCR 候选并人工确认。
7. OCR 确认前药箱不得新增库存；确认后新增库存，并验证两张临时图片已删除。
8. 修改密码后确认当前设备继续登录，旧 Refresh Token 无法再刷新；退出后回到登录页。
